...
public class MakebandDao {
	public int makeband(MakebandVo vo, long loginNum, String username) {
		...
		try {
			...
			int n = pstmt1.executeUpdate();
			 ...
			 if(rs.next()) { 
				 bandnum=rs.getLong("band_num"); 
			 }

			if (n > 0 && bandnum>0) {

				f = bandinfo(bandnum,loginNum, username);
				if (f > 0) {
					g = bandListinfo(bandnum,loginNum);
				}
			}
	...
	public int bandinfo(long bandnum,long loginNum, String username) {
		...
		try {
			...
			pstmt = con.prepareStatement(
					"INSERT INTO BAND_USERINFO VALUES(BANDUSERINFO_SEQ.NEXTVAL,?,?,?,1,SYSDATE)");
			pstmt.setLong(1, bandnum);
	...
	public int bandListinfo(long bandnum,long loginNum) {
